...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "config", postgresql.JSON(astext_type=sa.Text()), nullable=False, server_default="{}"
        ),
        sa.Column(
    ...
    op.create_table(
        ...
        sa.Column(
            "members", postgresql.JSON(astext_type=sa.Text()), nullable=False, server_default="[]"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "context", postgresql.JSON(astext_type=sa.Text()), nullable=False, server_default="{}"
        ),
        sa.Column(
            "tags", postgresql.JSON(astext_type=sa.Text()), nullable=False, server_default="[]"
        ),
    ...
    op.execute(
        "CREATE INDEX ix_memory_embedding ON memories_v2 USING ivfflat (embedding vector_cosine_ops)"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "shared_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")
        ),
        sa.Column("expires_at", sa.DateTime(timezone=True), nullable=True),
        sa.ForeignKeyConstraint(
            ["memory_id"],

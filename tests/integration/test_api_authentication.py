...
class TestUserRegistrationFlow:
    ...
    async def test_register_user_invalid_data(self, async_client: AsyncClient):
        ...
        invalid_data_sets = [
            ...
            # Missing email
            {"username": "testuser", "password": "secure_password_123"},
            # Missing password
            {"username": "testuser", "email": "test@example.com"},
            # Weak password
            {"username": "testuser", "email": "test@example.com", "password": "weak"},
            # Invalid email format
            {"username": "testuser", "email": "not_an_email", "password": "secure_password_123"},
        ]
...
class TestLoginFlow:
    ...
    async def test_login_invalid_credentials(self, async_client: AsyncClient, test_user_data):
        ...
        invalid_credentials = [
            # Wrong username
            {"username": "nonexistent_user", "password": test_user_data["password"]},
            # Wrong password
            {"username": test_user_data["username"], "password": "wrong_password"},
            # Both wrong
    ...
    async def test_login_locked_account(self, async_client: AsyncClient, locked_user):
        ...
        login_data = {"username": "locked_user", "password": "locked_password_123"}
...
class TestTokenRefreshFlow:
    ...
    async def test_token_refresh_used_token(
        self, async_client: AsyncClient, test_user, test_user_data
    ):
...
class TestAPIKeyAuthentication:
    ...
    async def test_api_key_invalid(self, async_client: AsyncClient):
        ...
        async_client.headers.update({"X-API-Key": "invalid.api.key.format"})
    ...
    async def test_api_key_list(self, authenticated_client: AsyncClient):
        ...
        await authenticated_client.post(
            "/auth/api-keys", json={"name": "List Test Key", "scopes": ["read"]}
        )
    ...
    async def test_api_key_revocation(self, authenticated_client: AsyncClient):
        ...
        create_response = await authenticated_client.post(
            "/auth/api-keys", json={"name": "Revoke Test Key", "scopes": ["read"]}
        )
...
class TestLogoutFlow:
    ...
    async def test_logout_without_auth(self, async_client: AsyncClient):
        ...
        logout_response = await async_client.post(
            "/auth/logout", json={"refresh_token": "any_token"}
        )
...
class TestPasswordManagement:
    ...
    async def test_change_password_weak_new(
        self, authenticated_client: AsyncClient, test_user_data
    ):
        """Test password change with weak new password."""
        change_data = {"current_password": test_user_data["password"], "new_password": "weak"}
...
class TestAuthenticationPerformance:
    ...
    async def test_login_performance(
        self, async_client: AsyncClient, test_user, test_user_data, performance_timer
    ):
    ...
    async def test_authenticated_request_performance(
    ...
    async def test_api_key_auth_performance(
        self, async_client: AsyncClient, test_api_key, performance_timer
    ):
...
class TestAuthenticationEdgeCases:
    ...
    async def test_case_sensitive_authentication(
        self, async_client: AsyncClient, test_user, test_user_data
    ):
        ...
        wrong_case_response = await async_client.post(
            "/auth/login",
            json={
                "username": test_user_data["username"],
...
class TestAPIKeyDependencyIntegration:
    ...
    async def test_verify_api_key_invalid_format(self, async_client: AsyncClient):
        ...
        response = await async_client.get("/health", headers={"X-API-Key": "invalid_no_dot"})
    ...
    async def test_verify_api_key_usage_tracking(self, async_client: AsyncClient, test_api_key):
        ...
        # Make request
        response = await async_client.get("/health", headers={"X-API-Key": api_key})
        assert response.status_code == 200
    ...
    async def test_verify_api_key_unlimited_rate_limit(
        self, async_client: AsyncClient, test_api_key
    ):
    ...
    async def test_verify_api_key_unlimited_expiration(
        self, async_client: AsyncClient, test_api_key
    ):

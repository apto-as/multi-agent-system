...
class TestCategoryInferencePerformance:
    ...
    def test_worst_case_performance(self):
        ...
        assert p95_time < 1.0, (
            f"Worst-case too slow: {p95_time:.3f}ms (target: <1.0ms)"
        )
    ...
    def test_regression_vs_10_category_baseline(self):
        ...
        print(f"  Baseline (10-cat) avg: ~0.5ms")
        print(f"  Baseline (10-cat) P95: ~1.0ms")
        ...
        assert p95_time <= 1.0, (
            f"P95 regression detected: {p95_time:.3f}ms > 1.0ms baseline"
        )

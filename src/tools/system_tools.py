    ...
    async def register_tools(self, mcp: FastMCP) -> None:
        ...
        async def get_system_status() -> dict[str, Any]:
            ...
            async def _get_system_status(session, services):
                ...
                return {
                    ...
                    "uptime_info": {"status_checked_at": datetime.utcnow().isoformat()},
                    "database": {
        ...
        async def health_check(
            ...
            return self.format_success(
                result,
                f"Health check completed - Status: {result.get('overall_status', 'unknown')}",
            )
                ...
                if request.optimize_vectors:
                    try:
                        ...
                        optimization_results["operations"]["vector_optimization"] = {
                            ...
                            "avg_dimensions": float(before_stats.avg_dimensions)
                ...
                if request.cleanup_logs:
                    try:
                        ...
                        optimization_results["operations"]["log_cleanup"] = {
                            ...
                            "logs_deleted": cleanup_result.rowcount
                            if hasattr(cleanup_result, "rowcount")
                            else 0,
            ...
            async def _get_performance_metrics(session, services):
                ...
                try:
                    ...
                    metrics["database_activity"] = {
                        ...
                        "avg_memory_age_hours": float(memory_stats.avg_age_hours)
                        if memory_stats and memory_stats.avg_age_hours
                        else 0,
                        "creation_rate_per_hour": (memory_stats.recent_memories / time_window_hours)
